...
CATEGORY_ROUTING: Dict[str, Any] = {
    "Broken Hospital Bed": {
        "sla_hours": 4,
        "first_responder": "Ward Nurse",
        "resolution_owner": "Maintenance Team",
        "escalation_chain": ["Ward In-Charge", "Maintenance Supervisor", "Hospital Engineering Head", "Hospital Administrator"],
        "secondary_departments": ["General Ward", "Orthopaedics", "Surgery"],
        "action_on_report": "Isolate bed immediately, arrange replacement bed if available, log in maintenance register",
    },
    "Crowded Hospital Waiting Room": {
        "sla_hours": 2,
        "first_responder": "Reception / OPD Staff",
        "resolution_owner": "Administration",
        "escalation_chain": ["OPD In-Charge", "Administrative Officer", "Medical Superintendent", "Hospital Director"],
        "secondary_departments": ["OPD", "Emergency", "Patient Services"],
        "action_on_report": "Deploy crowd management, open additional counters, arrange temporary seating, assess queue flow",
    },
    "Dirty Hospital Bathroom": {
        "sla_hours": 1,
        "first_responder": "Housekeeping Staff",
        "resolution_owner": "Housekeeping",
        "escalation_chain": ["Housekeeping Supervisor", "Facilities Manager", "Infection Control Officer", "Hospital Administrator"],
        "secondary_departments": ["Infection Control", "Facilities Management"],
        "action_on_report": "Immediate cleaning and sanitisation, repair broken fittings, restock toiletries, inspect other bathrooms",
    },
    "Empty / Unstaffed Nursing Station": {
        "sla_hours": 1,
        "first_responder": "Nursing Supervisor",
        "resolution_owner": "Nursing Management",
        "escalation_chain": ["Nursing Supervisor", "Duty Medical Officer", "Medical Superintendent", "Hospital Administrator"],
        "secondary_departments": ["Administration", "General Ward"],
        "action_on_report": "Deploy standby nurse immediately, review shift allocation, conduct inquiry if life-threatening gap occurred",
    },
    "Overflowing Hospital Trash (Outside)": {
        "sla_hours": 2,
        "first_responder": "Housekeeping Staff",
        "resolution_owner": "Waste Management Team",
        "escalation_chain": ["Housekeeping Supervisor", "Biomedical Waste Officer", "Facilities Manager", "Hospital Administrator"],
        "secondary_departments": ["Housekeeping", "Facilities Management"],
        "action_on_report": "Clear overflow, arrange additional bins, segregate biomedical waste properly, schedule urgent waste pickup",
    },
    "Rats / Rodent Infestation": {
        "sla_hours": 6,
        "first_responder": "Housekeeping Supervisor",
        "resolution_owner": "Pest Control",
        "escalation_chain": ["Housekeeping Supervisor", "Pest Control Vendor", "Infection Control Officer", "Hospital Administrator"],
        "secondary_departments": ["Infection Control", "Facilities Management"],
        "action_on_report": "Seal food items, alert infection control, deploy traps or bait, schedule urgent pest control treatment, inspect food storage",
    },
    "Torn Hospital Privacy Curtain": {
        "sla_hours": 8,
        "first_responder": "Ward Nurse",
        "resolution_owner": "Housekeeping",
        "escalation_chain": ["Ward In-Charge", "Housekeeping Supervisor", "Facilities Manager"],
        "secondary_departments": ["General Ward", "Facilities Management"],
        "action_on_report": "Replace curtain or arrange temporary screen, log in maintenance register, check adjacent bays",
    },
    "Unappetizing Hospital Food Tray": {
        "sla_hours": 3,
        "first_responder": "Ward Nurse",
        "resolution_owner": "Dietary Department",
        "escalation_chain": ["Ward Nurse", "Dietitian", "Kitchen Supervisor", "Patient Services Manager"],
        "secondary_departments": ["Kitchen", "Patient Services"],
        "action_on_report": "Replace meal if possible, note in patient record, report to kitchen supervisor, review diet chart accuracy",
    },
    "Unhygienic / Contaminated Hospital Food": {
        "sla_hours": 2,
        "first_responder": "Ward Nurse",
        "resolution_owner": "Dietary and Infection Control",
        "escalation_chain": ["Ward Nurse", "Infection Control Officer", "Kitchen Supervisor", "Medical Superintendent", "Hospital Administrator"],
        "secondary_departments": ["Infection Control", "Kitchen"],
        "action_on_report": "Immediately remove contaminated food, preserve sample for testing, alert infection control, inspect kitchen batch, monitor affected patients",
    },
    "Water Puddle on Hospital Floor": {
        "sla_hours": 1,
        "first_responder": "Housekeeping Staff",
        "resolution_owner": "Facilities Management",
        "escalation_chain": ["Housekeeping Staff", "Facilities Supervisor", "Safety Officer", "Hospital Administrator"],
        "secondary_departments": ["Housekeeping", "Safety"],
        "action_on_report": "Place wet floor warning signs immediately, mop and dry area, identify and fix source of water, check for electrical hazard nearby",
    },
}
...
class PipelineConfig:
    ...
    description_temperature: float = float(os.getenv("DESCRIPTION_TEMP", "0.1"))
...
class HospitalMultiLLMOrchestrator:
    ...
    def _category_filtered_context(self, contexts: List[Dict[str, Any]], category: str) -> str:
        """
        Return only RAG docs whose source filename matches the predicted category.
        Falls back to top-1 doc if no category-matched doc is found.
        ...
        # Use matched docs if found, otherwise fall back to top-1
    ...
    def _mistral_description(self, complaint: ComplaintInput, fusion: Dict[str, Any], contexts: List[Dict[str, Any]], structured: Dict[str, Any]) -> str:
        ...
        # Build an explicit forbidden-topics line so the model cannot confuse adjacent categories.
        _forbidden = [c for c in self.valid_categories if c != category]
        forbidden_line = (
            f"FORBIDDEN: Do NOT describe any of these: {', '.join(_forbidden[:6])}{'...' if len(_forbidden) > 6 else ''}. "
            f"If the retrieved context mentions any of these topics, ignore that part completely.\n"
        )
        ...
        system_prompt = (
            ...
            f"{forbidden_line}"
            f"Write a single complaint_description (1-2 sentences) that:\n"
            f"  - Describes the specific '{category}' issue observed, where it is, and what risk it poses.\n"
            f"  - Is written in formal clinical language.\n"
            f"  - Does NOT include recommendations or actions.\n"
            f"  - Does NOT start with 'The patient'.\n"
            f"  - Does NOT mention any issue type other than '{category}'.\n"
        ...
        user_prompt = (
            ...
            f"Complaint signal (use only the parts relevant to '{category}'):\n"
            f"{fusion['fused_text']}\n\n"
            f"{contextual_block}"
    ...
    def _get_routing_metadata(self, category: str) -> Dict[str, Any]:
    ...
    def process(self, complaint: ComplaintInput) -> Dict[str, Any]:
        ...
        routing_meta = self._get_routing_metadata(category)
        ...
        final_payload: Dict[str, Any] = {
            # --- Input echo (for frontend display) ---
            "name": complaint.name,
            "hospital_name": complaint.hospital_name,
            "ward": complaint.ward,
            "location": f"{complaint.hospital_name}, {complaint.ward}",
            "image_caption": complaint.image_caption,
            "voice_text": complaint.voice_text,
            ...
            "department": department,
            "complaint_description": complaint_description,
            # --- Routing metadata (from lookup table) ---
            "routing": {
                "primary_department": department,
                "secondary_departments": routing_meta["secondary_departments"],
                "first_responder": routing_meta["first_responder"],
                "resolution_owner": routing_meta["resolution_owner"],
                "escalation_chain": routing_meta["escalation_chain"],
                "sla_hours": routing_meta["sla_hours"],
                "action_on_report": routing_meta["action_on_report"],
            },
            # --- Pipeline metadata ---
