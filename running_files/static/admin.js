document.addEventListener('DOMContentLoaded', () => {
    ...
    filterUrgentBtn.addEventListener('click',   () => setFilter('urgent'));
    filterResolvedBtn.addEventListener('click', () => setFilter('resolved'));
    refreshBtn.addEventListener('click', () => {
        refreshBtn.textContent = '⏳';
        fetchComplaints().then(() => { refreshBtn.innerHTML = '&#8635; Refresh'; });
    });
    ...
    function startAutoRefresh() {
        ...
        autoRefreshTimer = setInterval(() => {
            fetchComplaints(true);  // silent = true (no "Loading..." flash)
        }, 15000);
    }
    ...
    async function fetchComplaints(silent = false) {
        ...
        try {
            ...
            console.error(err);
            if (!silent) {
                complaintList.innerHTML = '<p style="color:red;text-align:center;">Failed to load complaints. Is the server running?</p>';
            }
        }
    }
    ...
    function renderComplaints(complaints) {
        ...
        complaints.forEach(c => {
            ...
            const badgeClass = (c.severity || 'low').toLowerCase();
            ...
            card.innerHTML = `
                ...
                <div class="card-meta" style="margin-top:6px;">
                    SLA: ${c.sla_hours}h &bull; First Responder: ${c.first_responder || 'N/A'}
                </div>` : ''}
                ${c.relevant_sop ? `
                <div class="card-sop">
